...
def eval_dataset(
    ...
    nlls = []   # 存储每个输入片段的负对数似然值
    prev_end_loc = 0    # 上一个片段的结束位置，用于计算当前片段的起始位置

    idx = 0     #检索数据库索引
    all_token_ppls = []     # 存储每个token的困惑度
    all_tokens_to_predict = []      # 存储用于预测的目标token
    all_chosen_doc_ids = [None]     # 存储选择的文档ID
    num_inputs_no_retrieval = 0     # 计数器，用于跟踪没有检索到文档的输入数量
    # 遍历数据集
    for begin_loc in tqdm(range(0, dataset_len, stride)):
        end_loc = min(begin_loc + max_length, retrieval_dataset[-1]["end_location"], dataset_len)
        ...
        idx += 1
        if end_loc >= retrieval_dataset[-1]["end_location"]:
            break
