...
RETRIEVAL_FILE = "/home/zhangzhenzhi/Project/in-context/nq-test-dense-results.json"
OUTPUT_FILE = "nq-test-dense-results-top10-0.5.json"
        ...
        for entry in tqdm(batch, desc=f"Processing batch {i // BATCH_SIZE + 1}"):
            ...
            if not ctxs:
                ...
                for doc in ctxs[:10]:
                    try:
                        text = doc["text"]
                        compressed_prompt = llm_lingua.compress_prompt(text, rate=0.5, force_tokens=['\n', '?'])
                        compressed_text = compressed_prompt.get('compressed_prompt', "")
                    except Exception as e:
                        print(f"Error compressing doc: {e}")
                        compressed_text = ""
                    # 保存处理后的结果
