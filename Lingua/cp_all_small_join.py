...
'''
先拼接再压缩，压缩模型为small，保存文件为rf_lingua2small_xxx
小写rf
'''
...
RETRIEVAL_FILE = "/home/zhangzhenzhi/Project/in-context/$RETRIEVAL_FILE_dpr_32"
OUTPUT_FILE = "rf_lingua2small_dpr_32_3_0.57"
    ...
    for i in range(0, len(data), BATCH_SIZE):
        ...
        for entry in tqdm(batch, desc=f"Processing batch {i // BATCH_SIZE + 1}"):
            ...
            if not retrieved_docs:
                ...
                # 拼接前X个文档的文本内容
                combined_text = [doc['text'] for doc in retrieved_docs[:3]]

                compressed_prompt = llm_lingua.compress_prompt(combined_text, rate=0.57, force_tokens=['\n', '?'])
