...
'''
仅压缩首个数据，压缩模型为big，保存文件为retrieved_file_big_xxx
retrieved_file_
'''
...
RETRIEVAL_FILE = "/home/zhangzhenzhi/Project/in-context/$RETRIEVAL_FILE_con_32"
OUTPUT_FILE = "retrieved_file_big_con_32_0.5"
    ...
    for i in range(0, len(data), BATCH_SIZE):
        ...
        for entry in tqdm(batch, desc=f"Processing batch {i // BATCH_SIZE + 1}"):
            ...
            if not retrieved_docs:
                ...
                # 仅处理第一个文档
                first_doc_text = retrieved_docs[0].get('text', '')

                compressed_prompt = llm_lingua.compress_prompt(first_doc_text, rate=0.5, force_tokens=['\n','?'])
