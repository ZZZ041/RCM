...
'''
先拼接再压缩，压缩模型为big，保存文件为rf_lingua2big_xxx
小写rf
'''
...
OUTPUT_FILE = "rf_lingua2big_15_0.12"
    ...
    for i in range(0, len(data), BATCH_SIZE):
        ...
        for entry in tqdm(batch, desc=f"Processing batch {i // BATCH_SIZE + 1}"):
            ...
            if not retrieved_docs:
                ...
                # 拼接前X个文档的文本内容
                combined_text = [doc['text'] for doc in retrieved_docs[:15]]

                compressed_prompt = llm_lingua.compress_prompt(combined_text, rate=0.12, force_tokens=['\n', '?'])
